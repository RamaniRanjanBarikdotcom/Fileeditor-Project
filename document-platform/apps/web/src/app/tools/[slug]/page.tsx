  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-24 text-center space-y-4">
        ...
        />
        <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>Loading tool configuration...</p>
      </div>
    );
  }
  ...
  if (!tool) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-24 text-center space-y-4">
        <h1 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>Tool Not Found</h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>The requested tool does not exist or is currently unpublished.</p>
        <Link
          href="/tools"
    ...
    <div style={{ minHeight: '100vh' }}>
      ...
      >
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4">
          {/* Breadcrumbs */}
          <nav className="flex items-center justify-center gap-2 text-xs font-medium mb-4" style={{ color: 'var(--text-muted)' }}>
            <Link href="/" style={{ color: 'var(--text-muted)', textDecoration: 'none' }} className="hover:text-indigo-400">
              Home
            </Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <Link href="/tools" style={{ color: 'var(--text-muted)', textDecoration: 'none' }} className="hover:text-indigo-400">
              Tools
            </Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <span style={{ color: 'var(--text-primary)', fontWeight: 600 }}>{pres.name || tool.name}</span>
          </nav>
          ...
          >
            {tool.seoMetadata?.description || pres.features[0] || 'Fast, secure, and accurate online conversion engine. Convert directly in your browser.'}
          </p>
        </div>
      </section>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {slug === 'document-editor' ? (
          <div className="card p-10 text-center">
            ...
            <p className="mx-auto mb-7 max-w-xl text-sm leading-6" style={{ color: 'var(--text-muted)' }}>
              The studio runs inside your private workspace so exports and conversion history stay associated with your account.
            </p>
            <Link href="/app/editor" className="btn btn-primary btn-lg">Open Document Studio <ArrowRight className="h-4 w-4" /></Link>
          </div>
        ) : (
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-10">
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>
            Zero installation required. Works smoothly across Windows, macOS, Linux, and mobile browsers.
          </p>
        </div>
        ...
        >
          {pres.steps.map((s) => (
            ...
            >
              ...
              <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)', lineHeight: '1.6' }}>{s.desc}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          <div className="space-y-4">
            {pres.faq.map((item, idx) => (
              ...
              >
                ...
                <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', lineHeight: '1.7', paddingLeft: '1.5rem' }}>
                  {item.a}
                </p>
              </div>
            ))}
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        ...
        >
          <div className="relative z-10 space-y-4">
            ...
            <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: '0.9375rem', maxWidth: '520px', margin: '0 auto', lineHeight: '1.7' }}>
              Get 500 conversions per month, 100 MB max file sizes, and 30-day cloud retention for $9/month (or ₹749/month).
            </p>
            <div className="pt-2">
              <Link
                href="/pricing"
