...
const money = (price: Price | undefined, currency: Currency) =>
  price
...
export default function SoftwareDetailPage() {
  ...
  const checkout = async () => {
    ...
    const result = await fetchApi<Checkout>('/orders/checkout', {
      ...
      body: JSON.stringify({
        ...
        currency,
        successUrl: `${window.location.origin}/app/library?purchase=success`,
    ...
    if (!result.success || !result.data) {
      ...
      setError(
        message.includes('Unauthorized') ? 'Please sign in before starting checkout.' : message,
      );
      setBuying(false);
      return;
    }
    ...
    const gateway = new window.Razorpay({
      ...
      handler: async (payment: Record<string, string>) => {
        const verified = await fetchApi('/orders/verify-razorpay', {
          method: 'POST',
          body: JSON.stringify({
        ...
        if (verified.success) router.push('/app/library?purchase=success');
          ...
          setError(verified.error?.message || 'Payment verification failed.');
          setBuying(false);
        }
      },
  ...
  if (!product)
    return (
      <div className="mx-auto max-w-xl px-5 py-24 text-center">
        ...
        <h1 className="ts-h2">Product unavailable</h1>
        <p className="my-4" style={{ color: 'var(--text-muted)' }}>
          {error}
        </p>
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-12">
      <nav className="mb-10 flex items-center gap-2 text-xs" style={{ color: 'var(--text-muted)' }}>
        <Link href="/">Home</Link>
      ...
      <div className="grid items-start gap-10 lg:grid-cols-[1fr_380px]">
        <section>
          <span className="badge mb-5 bg-indigo-500/10 text-indigo-500">
            {product.type}
            {product.currentRelease ? ` · v${product.currentRelease.version}` : ' · preview'}
          </span>
          <h1 className="ts-h1 mb-5">{product.name}</h1>
          {product.tagline && (
            <p className="mb-4 text-xl font-semibold" style={{ color: 'var(--text-secondary)' }}>
              {product.tagline}
            </p>
          )}
          <p className="max-w-3xl text-base leading-8" style={{ color: 'var(--text-muted)' }}>
            {product.description}
          </p>
          <div className="card mt-10 p-7">
            <h2 className="ts-h3 mb-5">Purchase protection</h2>
            <ul className="space-y-4 text-sm" style={{ color: 'var(--text-secondary)' }}>
              <li className="feature-item">
                <CheckCircle2 className="h-4 w-4 text-emerald-500" /> Availability is checked
                against the current catalog release.
              </li>
              <li className="feature-item">
                <ShieldCheck className="h-4 w-4 text-emerald-500" /> Payment is verified by Stripe
                or Razorpay before access is granted.
              </li>
              <li className="feature-item">
                <Download className="h-4 w-4 text-emerald-500" /> Purchased releases use
        ...
        <aside className="card border-2 border-indigo-500/60 p-8 shadow-xl">
          <div className="mb-5 flex gap-2">
            {(['USD', 'INR'] as Currency[]).map((item) => (
              <button
                ...
                className={`rounded-lg px-4 py-2 text-xs font-bold ${currency === item ? 'bg-indigo-600 text-white' : 'bg-slate-500/10'}`}
              >
                {item}
              </button>
            ))}
          ...
          {error && (
            <div className="my-5 rounded-xl border border-rose-500/20 bg-rose-500/10 p-4 text-sm text-rose-500">
              {error}
            </div>
          )}
          <button
            ...
            className="btn btn-primary btn-lg mt-6 w-full disabled:cursor-not-allowed disabled:opacity-50"
          >
            {buying ? (
              <>
          ...
          {error.includes('sign in') && (
            <Link
              href={`/login?redirect=/software/${product.slug}`}
              className="mt-4 block text-center text-sm font-semibold text-indigo-500"
            >
              Sign in to continue
            </Link>
          )}
